...
```bash
# 콘솔 스크립트로 설치(선택) — 이후 어디서든 `yok3x <cmd>` 로 실행
pip install -e .            # 의존성 0, requires-python >=3.10

# 회귀 테스트 — mock 위에서 외부 CLI 없이 결정적으로 실행
pytest -q                   # 격리·버전 일관성·스톨·3패턴 E2E·가드 정지·BOM·verify_cmd
```

설치 없이도 `python3 yok3x.py <cmd>`(또는 `python -m yok3x.cli <cmd>`)로 바로 쓸 수 있다. 버전은 `yok3x/_version.py` 한 곳에서만 관리한다(패키지·배너·CLI·pyproject 공유).
