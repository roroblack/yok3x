...
| **LangGraph**(조건부 엣지) | **조건부 라우팅(에스컬레이션)** — 낮은 점수 지속 시 워커 1회 전환 | 정적 패턴 한계. 수렴 실패 시 다른/강한 워커로 동적 라우팅(스톨감지·열화의 일반화) | `orchestrator.run_producer_reviewer`(escalate), task spec `escalate` | `v4.0.0-plan-conditional-routing-2026-07-15.md` |
