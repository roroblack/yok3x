"""harness config: harness.json(전역 설정) + backends.json(백엔드 어댑터) 로딩.
...
DEFAULT_HARNESS = {
    ...
    "limits": {
        "claude": {
            ...
            "limit_7d_tokens": 0        # `harness calibrate`로 실사용 기반 정확 보정 권장
        },
        "codex": {
...
# 정확도는 `harness calibrate <tool> <5h|7d> <실제%>`로 본인 실사용에 맞춰 역산하는 것이 정답이다.
...
class Paths:
    ...
    def at(cls, root: str | Path = ".") -> "Paths":
        ...
        return cls(
            ...
            harness_json=root / "harness.json",
...
class Config:
    ...
    def load(cls, root: str | Path = ".") -> "Config":
        ...
        if p.harness_json.exists():
            harness = _deep_merge(harness, json.loads(p.harness_json.read_text(encoding="utf-8-sig")))
        if p.backends_json.exists():
            backends = _deep_merge(backends, json.loads(p.backends_json.read_text(encoding="utf-8-sig")))
        return cls(paths=p, harness=harness, backends=backends)

    def save_harness(self) -> None:
        self.paths.harness_json.write_text(
            json.dumps(self.harness, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    def worker(self, name: str) -> dict[str, Any]:
        w = self.harness["workers"].get(name)
        if not w:
            raise KeyError(f"unknown worker: {name} (available: {list(self.harness['workers'])})")
        return w

    def flavor(self) -> dict[str, Any]:
        f = self.harness["flavor"]
        return self.harness["flavors"][f]

    def ensure_dirs(self) -> None:
        for d in (self.paths.harness_dir, self.paths.runs, self.paths.logs, self.paths.knowledge):
...
def scaffold(root: str | Path = ".", use_mock: bool = False) -> Config:
    """`harness init`: 설정 파일과 디렉터리 생성."""
    cfg = Config.load(root)
    cfg.ensure_dirs()
    if use_mock:
        for w in cfg.harness["workers"].values():
            w["backend"] = "mock"
    if not cfg.paths.harness_json.exists():
        cfg.paths.harness_json.write_text(
            json.dumps(cfg.harness, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
