"""harness CLI.
  ...
  harness init [--mock]                프로젝트 초기화(설정·디렉터리 생성)
  harness setup                         "멀티에이전트 시스템 구성해줘" 자동 셋팅
  harness run <task.json> [--auto]      태스크 1회 실행(승인 게이트 포함)
  harness loop <task.json> -n N         에이전트 루프(가드가 스스로 정지)
  harness mat [--watch]                 사용량·코칭·진행 상태 한 화면
  harness coach                         사용량 코칭 메시지 출력
  harness coach guard on|off            요금 가드 on/off
  harness knot save|ingest|query|lint   지식그물
  harness flavor [이름]                 flavor 확인/변경
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(prog="harness",
                                description="하네스 멀티 에이전트 v2.2 — Claude Code · Codex · Gemini CLI 오케스트레이터")
    sub = p.add_subparsers(dest="cmd", required=True)

    sp = sub.add_parser("init", help="초기화: harness.json/backends.json/디렉터리 생성")
    ...
    if a.cmd == "init":
        ...
        print(f"초기화 완료: {cfg.paths.harness_json}, {cfg.paths.backends_json}")
        print(f"flavor={cfg.harness['flavor']}, guard={'ON' if cfg.harness['guard']['enabled'] else 'OFF'}"
    ...
    if a.cmd == "setup":
        ...
        print("실행 예: harness run task-producer-reviewer.json --auto")
        return 0
    ...
    if a.cmd == "coach":
        if a.args[:1] == ["guard"]:
            if a.args[1:2] == ["on"]:
                ...
                print("사용법: harness coach guard on|off"); return 2
            return 0
    ...
    if a.cmd == "plan":
        ...
        if a.tool and a.name:
            ...
            if a.name not in presets:
                ...
                return 2
            cfg.harness.setdefault("limits", {}).setdefault(a.tool, {})["plan"] = a.name
            cfg.save_harness()
            print(f"{a.tool} plan = {a.name}  {presets[a.name]}")
            print("정확도는 `harness calibrate claude 7d <실제%>` 로 보정 권장")
        else:
            for tool, presets in PLAN_PRESETS.items():
                cur = (cfg.harness.get("limits", {}).get(tool, {}) or {}).get("plan") or "(미설정)"
    ...
    if a.cmd == "calibrate":
        ...
        if a.percent <= 0:
            ...
            return 2
        conf = cfg.harness.get("limits", {}).get("claude", {})
        ...
        cfg.harness.setdefault("limits", {}).setdefault("claude", {})[key] = cap
        cfg.save_harness()
    ...
    if a.cmd == "flavor":
        if a.name:
            if a.name not in cfg.harness["flavors"]:
                print(f"없는 flavor: {a.name} (가능: {list(cfg.harness['flavors'])})")
                return 2
            cfg.harness["flavor"] = a.name
            cfg.save_harness()
        f = cfg.flavor()
        print(f"flavor={cfg.harness['flavor']}  orchestrator={f['orchestrator']}")
...
def entry() -> int:
    try:
        return main()
    except BrokenPipeError:      # `harness mat | head` 등 파이프 조기 종료
        return 0
