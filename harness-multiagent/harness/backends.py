...
def run_backend(name: str, spec: dict[str, Any], prompt: str) -> BackendResult:
    btype = spec.get("type", "cli")
    t0 = time.time()
    if btype == "mock":
        res = _run_mock(name, spec, prompt)
    elif btype == "cli":
        res = _run_cli(name, spec, prompt)
    elif btype == "native":
...
def _run_cli(name: str, spec: dict[str, Any], prompt: str) -> BackendResult:
