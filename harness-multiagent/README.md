...
python3 harness.py init --mock
python3 harness.py setup
python3 harness.py run task-producer-reviewer.json --auto
python3 harness.py mat

# 2) 실전: CLI 설치 후 mock 없이 초기화
python3 harness.py init          # backends.json이 claude/codex/gemini CLI를 호출
python3 harness.py loop task-pipeline.json -n 5   # 가드가 예산 초과 시 루프를 스스로 정지
```

`harness setup`이 샘플 태스크 3종을 생성한다("멀티에이전트 시스템 구성해줘" 자동 셋팅에 해당).

## 명령어

| 명령 | 기능 |
|---|---|
| `init [--mock]` | harness.json / backends.json / 디렉터리 생성 |
| `setup` | 샘플 태스크 3종 + 초기화 |
...
| mat 사용량·코칭 뷰 | `matview.py` — 환경변수 없이 harness.json 하나로 동작 |
...
| flavor별 오케스트레이터/워커 | `harness.json: flavors` — claude/codex/gemini-orchestrator 3종 |
| claude-main·codex-main·codex-critic·gemini 워커 | `harness.json: workers` — 역할 프롬프트 포함 |
| MCP·CLI·native 호출 + backends.json 어댑터 | `backends.py` — type: cli/native/mcp/mock |
| 승인 게이트·파일 로그·검증 체크리스트 | `orchestrator.py` — `.harness/runs/<id>/step_NN.json`, status.json |
...
| mat로 워커 진행 상태 확인 | `.harness/runs/*/status.json`을 mat이 집계 |
...
- `harness limits`로 각 probe의 라이브/stale/추정 원본을 확인한다.
...
```bash
harness plan claude max20x         # 요금제 프리셋(근사 상한 자동 적용). pro | max5x | max20x
harness calibrate claude 7d 30     # ← 정확: 지금 claude 앱 /usage 가 7일 30%면 이렇게 입력
```
...
```
harness.json  backends.json  context.md  brief.md  task-*.json
