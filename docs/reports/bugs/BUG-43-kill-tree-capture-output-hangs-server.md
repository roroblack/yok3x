...
## 여섯 번째 발견 — `list_models()`도 같은 캐시 스탬피드 (2026-08-06, 라이브 재현: 실제 30초 다운)

다섯 번째 수정을 배포하고 약 하루 지난 뒤, 재걸어둔 모니터가 **진짜 HTTP 다운**(연속 3회 실패,
포트는 열려 있는데 응답 없음, 30여 초 뒤 자연 복구)을 실측으로 잡아냈다. `py-spy` 덤프에 이번엔
`codex`가 아니라 `_gemini_bundle_models`(gemini 모델 목록 조회)에 여러 스레드가 동시에 멈춰 있었다.

**원인**: `list_models()`의 `_MODELS_CACHE`(TTL 5분)에도 `probe()`·`codex_percent_at()`과 똑같이
**락이 없었다.** 이 사용자 계정은 gemini API 키가 없어(Antigravity/CloudSDK 암호화 OAuth) 매번
`_gemini_bundle_models()`로 떨어지는데, 이 함수는 gemini-cli 번들 디렉터리의 `.js` 파일을 전부
`read_text`+정규식 스캔한다 — 가볍지 않은 I/O. 5분 TTL이라 스탬피드 창이 드물게 열리지만, 열릴 때
동시 요청이 몰리면(대시보드 폴링+동시 열람 등) 다들 캐시 미스를 보고 각자 이 무거운 스캔을 반복해
실제로 30초간 서버 전체가 응답 불능이 됐다(다섯 번째 발견까지 고친 codex 경로와는 무관한 별개
함수에서 재발한 **같은 문제 클래스**).

**수정**: `probe()`/`codex_percent_at()`과 동일한 백엔드별 락+더블체크락을 `list_models()`에 추가
(`_MODELS_CACHE_LOCKS`). claude/codex/gemini/local 네 백엔드 전부에 공통 적용.

**검증**: 신규 테스트(스레드 5개 동시 호출 → 실제 조회 1번만). 379 passed. 배포 후 서버 재확인 —
정상 응답.

**교훈**: 다섯 번째 발견 문서에 적어둔 "비슷한 무거운 I/O를 하는 함수는 전부 같은 취약점이 없는지
점검할 가치가 있다"는 경고가 실제로 맞아떨어졌다 — `codex_percent_at`을 고친 지 하루 만에 같은
모양의 버그가 `list_models`에서 실측으로 터졌다. 이 저장소에 "TTL 캐시" 패턴이 몇 군데 더 있다면
(`_OAUTH_LIVE_CACHE` 등) 전수 점검할 가치가 있다 — 지금은 실제로 다운을 유발한 것만 대응.
