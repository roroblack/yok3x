...
상속되니 producer가 그 테스트를 통과하는 한 `verify_ok=False`가 될 수 없는 구조였다. 3차
프로토콜을 같은 방식(동일인이 스펙+테스트 작성)으로 반복해도 같은 이유로 또 0건이 나올
가능성이 높다 — 문서에 재검토 옵션 3가지(스펙·테스트 작성자 분리 / hidden test / T-1
정의 자체를 "리뷰어 결함 적중률"로 재정의) 정리, 실행은 안 함(사용자 판단 필요).
