...
**계정 관계(확정, TFDEV 세션의 DB 조회로 확인 — 이쪽 도구로는 확인 불가라 출처 명시)**:
`[내부 테이블]`에 `teamflow`·`teamflow-mcp`·**`yok3x`(09-05 생성)** 세 계정이 실재하고, [내부]의
멤버는 `teamflow`·`teamflow-mcp`뿐이다. yok3x는 [내부] 하나에만 속한다. 따라서 `list_projects`에
81이 안 나오는 건 정상 동작이다.

**방법에 대한 정정**: 종전 진단서는 결론("yok3x는 별개 계정")은 맞았지만 **근거가 이름 두 개를
보고 한 추론**이었다. `list_projects`는 토큰이 닿는 프로젝트만 알려줄 뿐 **소유 계정은 안
알려준다** — 확인한 게 아니라 넘겨짚은 것이다.

**아직 유효한 오류 — 책임 귀속의 방향**: "TFDEV 쪽 배선만 빠졌다"는 근거가 없다. 저쪽이
`env: {}`로 폴백한 건 맞지만 **그 공유 `.env`에 yok3x 토큰을 넣은 건 이쪽(9/5 V-6 스모크
테스트)**이다. 한쪽만 지목한 게 오류다. `env.js`의 폴백도 버그가 아니라 의도된 설계다(주석에
이유 명시, TFDEV-148).
