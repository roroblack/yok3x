...
데이터·시간이 쌓여야 판단 가능한 항목을 여기 모은다. **하루 1회** `scripts/todo_check.py`가
자동 점검해 맨 아래 '자동 점검 상태'를 갱신한다(읽기 전용 계측 — 판단은 사람이).
...
- **작업E**: 리뷰어가 verify 결과를 **본 뒤** 채점하던 **라벨 누출**을 제거(blind 채점).
  → 이제부터 쌓이는 데이터는 비오염. **단, 작업E가 실제로 개선인지는 아직 미검증**(효과 측정 0).
...
세 조건을 **모두** 충족하면 점검 착수. 데이터는 `verify_cmd`가 설정된 실제 producer-reviewer
런에서만 쌓인다(그런 런을 돌려야 축적됨).
