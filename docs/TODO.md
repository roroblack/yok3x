...
## T-3. 수정 적용 방식 결정 — auto-commit(ratcheting) vs 파일게시+사람수락 (2026-07-24 등록, 설계 판단 대기)
...
**착수 조건**: 이건 즉시 결정 아님(장기 판단). R-7(worktree) 진행 시점에 1번 여부를 같이 결정.
현재 판단 보류 — 로드맵상 F2-2 → R-2+R-6 → R-3/R-4 먼저.
