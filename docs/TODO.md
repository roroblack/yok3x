...
| ~~3~~ | ~~**MCP 토큰 배선 고정**~~ | YOK-88 | **해결(2026-09-06, `c830c00`)** — 등록을 `~/.claude.json`(git 미추적)으로 옮기고 거기 토큰을 명시. 아래 "운영 위험" 갱신본 참고 |
...
### 운영 위험 — MCP 토큰 배선 (2026-09-06 진단 → 조치 완료, 잔여 위험은 반대편)
...
**yok3x 쪽은 끝났다.** `.mcp.json`은 git에 추적되므로 토큰을 거기 두지 않고, 등록 자체를
개인 전역 설정 `~/.claude.json`(git 미추적)으로 옮겨 `env.TEAMFLOW_API_TOKEN`에 자기 토큰을
명시했다(`c830c00`). 이제 공유 `.env`를 누가 갈아도 이쪽은 안 흔들린다.

**잔여 위험은 반대 방향이다.** `jira_for_me` 쪽 등록은 여전히 `env: {}`라 공유 `.env`를
물려받는데, 그 파일에는 지금 **yok3x 계정 토큰**이 들어 있다(2026-09-06 13:49 갱신).
즉 TFDEV 세션이 다시 남의 계정으로 붙어 403을 맞는 구성이다 — 2026-09-05 사고와 같은 방향.
조치는 TFDEV 소관(자기 `env`에 [내부 계정] 토큰 명시)이고, 이 저장소 토큰으로는
TFDEV에 티켓을 못 만들어 여기 기록만 남긴다.
