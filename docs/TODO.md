...
| 6 | **knot 노트 검증 상태**(`status`·`verified`·`stale_after`) — 먼저 주입 노트 id 계측 | YOK-119 | 계획서 완료, 구현 전. [계획서](plans/v4.x-plan-knot-verification-state-2026-09-21.md) |
| 7 | **워커 호출 전 verify 프리플라이트** | YOK-120 | **보류(근거 부족)** — 필요 사례 관찰 후 최소안(관측 전용). [계획서](plans/v4.x-plan-verify-preflight-2026-09-21.md) |
| 8 | **런북·관측 로그 저장 범위** | YOK-121 | 사용자 결정 대기(권장: 집계 명령 먼저). [결정 문서](plans/v4.x-plan-observation-store-scope-2026-09-21.md) |
