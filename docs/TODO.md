...
**정정 — 계정 귀속에 대한 종전 기술은 근거가 없었다.** 이 문서와 진단서는 한동안 "yok3x는
자기 계정 토큰을 쓰므로 정상이고 TFDEV 쪽 배선만 빠졌다"고 적고 있었다. 그 전제를 확인한
적이 없다. 확인된 건 `list_projects`가 [내부]만 admin으로 반환한다는 것뿐이고, **`list_projects`는
토큰이 닿는 프로젝트만 알려줄 뿐 소유 계정은 안 알려준다.** 사용자 확인 결과 실제로는
teamflow 계정 토큰을 공유해 쓰고 있었다(그 계정 토큰을 폐기하니 이 세션이 즉시 멈췄다).

**남은 사실 하나**(미해결): 그 계정이라면 `list_projects`에 [내부]도 나왔어야 하는데 한 번도
나온 적이 없다. yok3x 쪽 도구로는 확인 불가라 사실만 기록한다.

**교훈**: "토큰이 닿는 범위"와 "토큰의 소유 계정"은 다른 값인데 도구 표면에는 앞의 것만
보인다. 뒤의 것을 앞의 것으로 추정해 결론을 쓰지 않는다.
