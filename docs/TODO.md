...
- **F1-b**: 현재 verify는 후보가 아닌 원본 트리를 검사하므로 `verify_scope="original_tree"`로 기록하고
  캘리브레이션 라벨에서 제외한다. 유효 라벨 축적은 후보를 스테이징하는 **F1-d 완료가 선행조건**이다.
...
선행조건: verify가 후보를 검증해야 한다(**F1-d 스테이징 완료**). 그 전까지 `calibration.jsonl`의
`verify_ok`는 `verify_scope="original_tree"`이므로 SCORE↔verify 지상진실 라벨로 사용할 수 없다.
...
F1-d 선행조건과 세 조건을 **모두** 충족하면 점검 착수. 유효 데이터는 후보 스테이징 후
