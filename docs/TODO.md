...
**남은 설계 질문(계획서 승격 전 확정 필요)**:
1. yok3x task와 TeamFlow issue를 정확히 어떻게 매핑하는가 — task.json이 issue ID를
   참조하고, 오케스트레이터(또는 워커)가 라운드 진행에 따라 `update_issue_fields`로 상태를
   갱신(대기중→진행중→완료)하는 방식이 유력해 보이지만 확정 필요.
2. 지금 TeamFlow MCP 서버엔 댓글 작성 도구가 없다 — "무엇을 했는지" 요약을 사람이 보게
   하려면 issue 필드(설명란 등)를 쓸지, 아니면 jira-for-me 쪽에 토큰 스코프의 댓글 경로를
   새로 열어달라고 할지(그건 yok3x가 아니라 그 프로젝트의 별도 작업).
3. 여러 yok3x producer/reviewer 라운드를 TeamFlow의 서브태스크(Epic▸Story▸Sub-task)
   계층에 대응시킬지, 아니면 이슈 하나로 단순하게 둘지.

**검토 후보**: 위 3개를 좁힌 뒤 계획서로 승격 — a1(MCP)의 실사용 검증도 자연스럽게 같이
된다(T-4 참고, 지금까지 실사용 0건이었음).
