...
`docs/HISTORY.md`에서 롤링(RULE §7.3)한 오래된 항목. **날짜 범위**: 2026-07-04 ~ 2026-07-15 · **버전 범위**: v2.2.0 ~ v4.0.0 · **엔트리**: 40개. 최신 이력은 `docs/HISTORY.md`.

---

## 미출시(dev) · 2026-07-15 — effort 백엔드별 레벨(codex xhigh) + 워크스페이스 x-지우기 (사용자 요청)

- effort 옵션이 [low/medium/high] 하드코딩이던 것을 **백엔드별 실제 지원 레벨**로 정정(라이브 검증):
  codex=**minimal/low/medium/high/xhigh**(에러메시지로 확인), claude=low/medium/high. guiserver 검증도 확장.
- 워크스페이스: **✕ 지우기 버튼**(설정 시 노출) 추가 + pickWorkspace/clearWorkspace가 **즉시 저장**
  (기존엔 선택만 하고 저장 안 해 다음 렌더에 유실되던 문제도 해소).
- RULE §5.6(UI/UX 변경 규칙)은 이미 존재함을 확인. 74 passed.

## 미출시(dev) · 2026-07-15 — 작업 관리 UI 통합(작업별 보기 옆으로) (사용자 요청)

- 흩어져 있던 작업 관리를 통합: '작업별 보기' 필터 = **작업 선택기**로, 그 옆에 [열기][▶ 실행][🗑]
  [💾 새 작업 저장] 배치. 하단의 별도 '저장된 작업' 드롭다운(c-regtask)·CRUD 행 **제거**.
- 필터가 이미 저장된 작업(💾)+런 라벨을 통합 표시하므로, 선택 라벨→저장 작업 파일명 매핑
  (`selectedSavedTaskName`)으로 열기/실행/삭제. 전송으로 만든 작업도 같은 곳에 나옴(일관).
- 라이브 검증: 상단 관리바 버튼·필터 통합·선택→파일 매핑 확인, c-regtask 참조 0. JS 에러 0.
...
- producer-reviewer에 **에스컬레이션**: task spec `escalate={after_round, if_score_below, to_producer, to_reviewer}`.
  낮은 점수가 지속되면 지정 라운드 이후 워커를 **1회 전환**(다른/강한 워커로). 정적 패턴 → 동적 라우팅.
- codex 리뷰(SCORE 7/10) 반영: 다음 라운드부터 적용·정확히 1회(escalated 플래그)·**대상 워커 사전검증**
  (부재 시 RunAborted, 조용한 폴백 금지)·**prev_sig 초기화**(워커 전환 시 스톨 오탐 방지)·교차패밀리 유지.
- opt-in(escalate 없으면 현행 불변). 테스트 2(전환 발동·부재 대상 실패). ADOPTIONS에 채택 기록. 73 passed.

## 미출시(dev) · 2026-07-15 — 작업별 보기 ↔ 저장된 작업 통합(따로 놀던 것 연결)

- 버그: '작업별 보기' 필터는 **런 라벨**만, '저장된 작업' 드롭다운은 **task 파일**만 → 저장했는데 안 돌린
  작업이 작업별 보기에 안 나오는 등 두 목록이 따로 놀았다.
- 수정: build_state에 `saved_tasks`([{name,label}]) 추가. GUI 필터를 **저장된 작업 라벨 ∪ 런 라벨**로 통합
  (저장된 작업은 `💾` 표시), 저장 안 돌린 작업 선택 시 "저장됨·실행 없음" 안내. 저장된 작업 드롭다운도
  파일명 대신 **라벨** 표시. 라이브 검증(저장→양쪽 목록 동시 등장), JS 에러 0. 신규 테스트 1.
