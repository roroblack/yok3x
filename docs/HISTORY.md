...
- v4.x · 2026-09-06 — **정정: 토큰 계정 귀속 오진**(TeamFlow YOK-96). 9/6 진단서는 "yok3x는
  ...
  **`list_projects`는 토큰이 닿는 프로젝트만 알려줄 뿐 소유 계정은 안 알려준다** — 이 둘을
  같은 값으로 취급한 게 직접 원인이다. 진단서는 자기모순이었다(맨 아래 "미해결"에 *토큰이
  어느 계정인지 도구 표면에 안 보인다*고 써놓고 결론에서 계정을 단정). 사용자 확인 결과
  teamflow 계정 토큰을 공유해 쓰고 있었다(그 계정 토큰 폐기 시 세션 즉시 정지). 진단서에
  정정 배너를 달고 TODO의 "운영 위험" 절을 다시 썼다. 미해결로 남는 사실: 그 계정이라면
  `list_projects`에 [내부]도 나왔어야 하는데 한 번도 나온 적이 없다(이쪽 도구로 확인 불가).
