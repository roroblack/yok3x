...
- v4.x · 2026-09-06 — **정정: 토큰 계정 귀속 오진**. 9/6 진단서가 계정 귀속을 확인 없이
  단정했다. `list_projects`는 토큰이 닿는 프로젝트만 알려줄 뿐 **소유 계정은 안 알려주는데**
  둘을 같은 값으로 취급했다. 진단서 자체가 자기모순이었다("어느 계정인지 도구 표면에 안
  보인다"고 써놓고 결론에서 계정을 단정). 교훈: 관측 불가라고 적어둔 값에 대해 결론을 쓰지
  않는다. 상세는 `docs/private/`(저장소 미포함).
...
- v4.x · 2026-09-06 — TeamFlow(프로젝트 YOK) 전체 백필: 그때까지 이슈가 4개뿐이라 작업 이력을
  ...
  **일자별 태스크 21개**(HISTORY의 모든 작업일 — 07-21부터 09-06까지, 각 날짜의 항목을 전부
  기술), **디버그노트 54건**(BUG-01~54를 개별 bug 이슈로, 심각도·커밋·연관 버그 포함),
  **열린 작업 9건**(V-11 UI·V-11 e2e·MCP 토큰 배선·V-6 확장·T-4(a2)·T-6·V-5§4.2·V-8·V-9)을
  등록했다. 완료되지 않은 것은 상태로 구분했다 — BUG-42(완화만)·BUG-54(경고만, 자동 차단
  아님)는 done이 아니라 inprogress.

- v4.x · 2026-09-06 — MCP 토큰 배선 진단: 두 저장소의 MCP 등록이 자기 토큰을 안 넘겨 공유
  `.env` 폴백 하나를 같이 쓰고 있었다. 등록마다 `env`에 자기 토큰을 명시하면 사라지는 문제다.
  RULE §9에 반영. 상세는 `docs/private/`(저장소 미포함).
