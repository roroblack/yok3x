...
| `yok3x-v4.4.0.zip` | 4.4.0 | 122 | 버전 표기를 계획서 번호에 정렬(3.6.0 → 4.4.0 — 그간 v3.7~v4.4 계획이 코드에 반영됐으나 버전을 올리지 않아 생긴 격차 해소). 내용: v3.6.0 이후 누적분 전체 — **조건부 라우팅**·**MCP 워커 도구**·**ACQUIRE 사전질의**·**F1 게이트/후보 스테이징/review bundle**·**G-1 재개**·**knot 그래프**·**nightly 자기개선 계획**, 그리고 v4.4.0 리서치 흡수 묶음(**R-1 테스트 격리**·**F2-2 종료코드 분리**·**R-2 새 증거 기반 정지규칙+stop_reason**·**R-6 verifier 불변성**·**R-3 preflight 예산 검사**·**R-4 `--json` 스냅샷+provenance enum**). 페이싱 정확도 하드닝(BUG-26~38, 특히 OAuth 스냅샷 붕괴·상한 드리프트 수정, 트랜스크립트 파싱 캐시로 대시보드 13s→0.4s). 336 tests. 폴더 스냅샷: `backup/yok3x-v3.6.0-20260726-1414/`(상향 직전 상태). |
